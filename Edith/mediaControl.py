...
edithPort = input("Enter EDITH port : ")
edithPort = "COM" + edithPort
...
def mode1():
    while True:
        ...
        if response == "mode2":
            mode2()
        elif response == "playpause":
            pyautogui.press("playpause")
        elif response == "mute":
            pyautogui.press("prevtrack")
        elif response == "sw1":
            pyautogui.hotkey("winleft", "ctrlleft", "t");
        elif response == "sw2":
            pyautogui.hotkey("winleft", "shiftleft", "c");
        elif response == "sw3":
...
#     board.write(bytes(now.strftime("%m/%d/%Y") + "\n", "utf-8"));
#     threading.Timer(5.0, sendingTime).start()
